...
using TechagroApiSync.Amaparts.Helpers;
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    foreach (var product in products)
                    {
                        ...
                        {
                            Log.Error(ex, $"Failed to update/insert product: Product EAN = {product.ProductCode}, Name = {product.ProductName}");
                        }
                        ...
                        {
                            Log.Error(ex, $"Failed to update/insert product description: Product EAN = {product.ProductCode}, Name = {product.ProductName}");
                        }
                            ...
                            if (product.Photos != null)
                            {
                                foreach (var imgUrl in product.Photos.Where(u => !string.IsNullOrEmpty(u)))
                                {
                                    try
                                    {
                                        ...
                                        await _productRepo.UpsertProductImageAsync(product.ProductCode, product.ProductCode.ToString(), imageData);
                        ...
                        {
                            Log.Error(httpEx, $"Failed to download product image: Product EAN = {product.ProductCode}, Name = {product.ProductName}");
                        }
                        catch (Exception ex)
                        {
                            Log.Error(ex, $"Failed to update/insert product image: Product EAN = {product.ProductCode}, Name = {product.ProductName}");
                        }
                    }
                    Log.Information("Products imported: {Total} out of {ToUpdate}, Inserted: {Inserted}, Updated: {Updated}", productInserted + productUpdated, products.Count(), productInserted, productUpdated);
                }
                catch (Exception ex)
                {
